<template>
  ...
  <main class="flex flex-col h-screen">
    ...
    <section
      class="w-full h-full flex justify-center items-center relative z-30"
    >
      <div
      ...
      >
        ...
        >
          <header class="flex items-center justify-between font-bold">
            <span class="uppercase text-xs text-[#401E79]">
              {{
                formatDate(
                  raffleDetail.initialDate || new Date().toDateString()
                )
              }}
            ...
            <span class="text-xs text-[#401E79]"
              >Total: {{ raffleDetail._count.Participant || 0 }}</span
            >
          </header>
          <div class="flex flex-row items-center space-x-3">
            ...
            <p class="text-base text-[#401E79] font-semibold">
              {{ raffleDetail.name }}
            </p>
          </div>

          <p class="font-medium text-[#401E79]">
            {{ raffleDetail.description }}
          </p>

          <section
            class="grid grid-cols-4 w-full h-fit gap-5 px-5"
            v-if="!raffleDetail.isFinished"
          >
            <div class="h-fit flex flex-col gap-3 col-span-3">
              <button
                @click="goToChooseWinner()"
          ...
          <section class="grid grid-cols-4 w-full h-fit gap-5 px-5" v-else>
            <div class="h-fit flex flex-col gap-3 col-span-3">
              <p class="font-semibold text-[#401E79]">
                Ganador(s): {{ winnersName.join(', ') }}
              </p>
            </div>
          </section>
        </article>
        ...
        <h1
          class="text-start text-xl md:text-2xl xl:text-2xl 2xl:text-2xl font-bold my-5 text-white"
        >
          Participantes Registrados
        </h1>
        <section class="w-full" v-if="participantList.length">
          <article
          ...
          >
            ...
            <header class="flex flex-col items-start font-bold gap-2">
              ...
              <span class="text-xs text-[#401E79]">{{
                participant.idPlatform
              }}</span>
            </header>
          </article>
        </section>

        <section class="flex justify-center items-center w-full mt-4" v-else>
          <p class="font-semibold text-white">No hay participantes</p>
        </section>
      </div>
    </section>
  </main>
...
const { raffleDetail, findOneRaffle, shareUrlInClipboard, winnersName } =
  useRaffle();
